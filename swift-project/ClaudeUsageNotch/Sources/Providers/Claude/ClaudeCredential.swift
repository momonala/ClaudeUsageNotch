...
/// The browser session cookie, used only when `~/.claude/credentials.json`
/// yields no OAuth token — see `ClaudeOAuthCredential`.
///
/// The entire cookie string is stored (not just `sessionKey=…`) because
/// `claude.ai`'s usage endpoint also reads `lastActiveOrg` and other hardening
/// cookies. Treat this value as a secret — it is keychain-only.
